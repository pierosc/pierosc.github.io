...
test("renderiza la experiencia cinematográfica", async () => {
  ...
  assert.doesNotMatch(html, /IMAGEN PENDIENTE/i);
  assert.match(html, /\/images\/story\/ep3-mustafar\/01\.webp/i);
  assert.match(html, /\/images\/story\/ep5-bespin\/03\.webp/i);
  assert.match(html, /\/images\/story\/ep6-redencion\/02\.webp/i);
